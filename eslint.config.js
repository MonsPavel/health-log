...
const allowExternalModule = (name) => ({ to: { origin: "external" }, dependency: { module: name } });
...
const elements = [
  ...
  { type: "domain", pattern: `${MODULE_ROOT}/*/domain/**`, capture: ["module"], partialMatch: false },
  { type: "application", pattern: `${MODULE_ROOT}/*/application/**`, capture: ["module"], partialMatch: false },
  { type: "adapters", pattern: `${MODULE_ROOT}/*/adapters/**`, capture: ["module"], partialMatch: false },
...
const EVERYTHING_INTERNAL = [
  "renderer", "shared", "domain", "application", "adapters", "module-root",
  "main-app", "kernel", "contracts", "scales-data", "tools", "scripts",
].map(allowEl);
...

  {
    ...
    rules: {
      ...
      "no-restricted-syntax": [
        ...
        {
          selector: "CallExpression[callee.name='eval'], CallExpression[callee.property.name='eval']",
      ...
      "boundaries/dependencies": [
        "error",
        {
          ...
          policies: [
            ...
            { from: el("shared"), to: [el("kernel"), el("contracts")], allow: [allowEl("kernel"), allowEl("contracts")] },
            {
            ...
            { from: el("domain"), to: [el("kernel"), el("module-root")], allow: [allowEl("kernel"), allowEl("module-root")] },
            {
              from: el("application"),
              to: ownModule("domain"),
            ...
            },
            {
              ...
              allow: [allowEl("kernel"), allowEl("contracts"), allowEl("shared"), allowEl("module-root")],
            },
            // main/app: application + adapters модулей (любых), contracts, shared
            {
              from: el("main-app"),
              to: [el("application"), el("adapters"), el("module-root"), el("contracts"), el("shared"), el("kernel")],
              allow: [
                allowEl("application"), allowEl("adapters"), allowEl("module-root"),
                allowEl("contracts"), allowEl("shared"), allowEl("kernel"),
              ],
            },
            ...
            { from: [el("tools"), el("scripts")], to: EVERYTHING_INTERNAL.map((e) => e.to), allow: EVERYTHING_INTERNAL },

            // --- внешние npm (checkAllOrigins) ---
            {
              from: el("domain"),
              allow: [allowExternalModule(["@hl/kernel", "@hl/contracts"])],
              message: "domain импортирует только @hl/kernel (и type-only @hl/contracts) — арх. 03 §4",
            },
            ...
            {
              from: [el("renderer"), el("shared"), el("application"), el("adapters"), el("module-root"), el("main-app"), el("tools"), el("scripts")],
              allow: [allowExternal],
            },
          ],
        },
      ...
      "no-restricted-imports": [
        "error",
        {
          paths: [
            ...
            {
              name: "better-sqlite3",
              message: "SQLite живёт в main-процессе (арх. 04); renderer работает через IPC и @hl/contracts",
            },
          ],
          patterns: [
            {
              group: ["node", "node:*"],
              message: "renderer без Node (арх. 08 §4); данные — через preload-мост и @hl/contracts",
            },
          ],
        },
      ],
