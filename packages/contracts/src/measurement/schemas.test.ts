...
function firstMessage(
  schema: {
    safeParse(p: unknown): { success: boolean; error?: { issues: { message: string }[] } };
  },
  payload: unknown,
): string {
...
describe('MEASUREMENT_ADD_REQUEST_SCHEMA — валидные payload (§11)', () => {
  ...
  it('принимает payload без опциональных pulse и note (FR-1.1: ЧСС опциональна)', () => {
    const minimal = copyWithout(copyWithout(VALID_ADD, 'pulse'), 'note');
  ...
  it('границы СДА/ДДА/ЧСС — включительно (SRS FR-1.2: 50–300, 20–200, 20–300)', () => {
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, sys: 50, dia: 20, pulse: 20 })
        .success,
    ).toBe(true);
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, sys: 300, dia: 200, pulse: 300 })
        .success,
    ).toBe(true);
  });
  ...
  it('границы tzOffsetMin [-720, +840] включительно (§13: UTC−12…+14)', () => {
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({
        ...VALID_ADD,
        takenAt: { utcMs: NOW_MS, tzOffsetMin: -720 },
      }).success,
    ).toBe(true);
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({
        ...VALID_ADD,
        takenAt: { utcMs: NOW_MS, tzOffsetMin: 840 },
      }).success,
    ).toBe(true);
  });
  ...
  it('заметка ровно 500 символов — ок (§13: 500 ок, 501 — NOTE_TOO_LONG)', () => {
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, note: 'а'.repeat(500) }).success,
    ).toBe(true);
  });
...
describe('MEASUREMENT_ADD_REQUEST_SCHEMA — нарушения дают конкретные ошибки (§19)', () => {
  it('СДА вне [50, 300] → message errors.rangeSys (границы 49/301)', () => {
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, sys: 49 })).toBe(
      'errors.rangeSys',
    );
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, sys: 301 })).toBe(
      'errors.rangeSys',
    );
  });

  it('нецелое СДА → errors.rangeSys (порядок домена §7: «целые → диапазоны»); не-число — просто отказ', () => {
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, sys: 120.5 })).toBe(
      'errors.rangeSys',
    );
    expect(MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, sys: '120' }).success).toBe(
      false,
    );
  });

  it('ДДА вне [20, 200] → errors.rangeDia (границы 19/201)', () => {
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, dia: 19 })).toBe(
      'errors.rangeDia',
    );
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, dia: 201 })).toBe(
      'errors.rangeDia',
    );
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, dia: 80.5 })).toBe(
      'errors.rangeDia',
    );
  });

  it('ЧСС вне [20, 300] → errors.rangePulse (границы 19/301, нецелое)', () => {
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, pulse: 19 })).toBe(
      'errors.rangePulse',
    );
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, pulse: 301 })).toBe(
      'errors.rangePulse',
    );
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, pulse: 70.5 })).toBe(
      'errors.rangePulse',
    );
  });

  it('sys ≤ dia → errors.sysLeDia (инвариант VO BloodPressure, TASK-016)', () => {
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, sys: 120, dia: 120 })).toBe(
      'errors.sysLeDia',
    );
    expect(firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, sys: 80, dia: 120 })).toBe(
      'errors.sysLeDia',
    );
  });
  ...
  it('заметка 501 символ → errors.noteTooLong (§16–17)', () => {
    expect(
      firstMessage(MEASUREMENT_ADD_REQUEST_SCHEMA, { ...VALID_ADD, note: 'а'.repeat(501) }),
    ).toBe('errors.noteTooLong');
    expect(MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, note: 42 }).success).toBe(
      false,
    );
  });

  it(`плохой arm → отказ; 'left'/'right' проходят (синхронизация с ARM_VALUES, TASK-016)`, () => {
    expect(MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, arm: 'center' }).success).toBe(
      false,
    );
    expect(MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, arm: 'LEFT' }).success).toBe(
      false,
    );
  ...
  it('irregularPulse — только boolean (§13: флаг без ограничений-валидации)', () => {
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, irregularPulse: 'no' }).success,
    ).toBe(false);
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, irregularPulse: true }).success,
    ).toBe(true);
  });
  ...
  it('takenAt.utcMs — только целое; «не будущее» схемой НЕ проверяется (§13: вердикт домена)', () => {
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({
        ...VALID_ADD,
        takenAt: { utcMs: NOW_MS + 0.5, tzOffsetMin: 180 },
      }).success,
    ).toBe(false);
    ...
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({
        ...VALID_ADD,
        takenAt: { utcMs: NOW_MS + 365 * 86_400_000, tzOffsetMin: 180 },
      }).success,
    ).toBe(true);
  });
  ...
  it('tzOffsetMin вне [-720, 840] → отказ (границы −721/841)', () => {
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({
        ...VALID_ADD,
        takenAt: { utcMs: NOW_MS, tzOffsetMin: -721 },
      }).success,
    ).toBe(false);
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({
        ...VALID_ADD,
        takenAt: { utcMs: NOW_MS, tzOffsetMin: 841 },
      }).success,
    ).toBe(false);
  });

  it(`profileId — непустая строка ≤64 (§14); '' и 65 символов отклоняются`, () => {
    expect(MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, profileId: '' }).success).toBe(
      false,
    );
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, profileId: 'п'.repeat(65) }).success,
    ).toBe(false);
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, profileId: 'п'.repeat(64) }).success,
    ).toBe(true);
  });

  it('strict: лишнее поле на верхнем уровне и внутри takenAt отклоняется (§14: IPC-гигиена)', () => {
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({ ...VALID_ADD, source: 'manual' }).success,
    ).toBe(false);
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse({
        ...VALID_ADD,
        takenAt: { ...VALID_ADD.takenAt, wall: 'x' },
      }).success,
    ).toBe(false);
  });
  ...
  it('обязательные поля отсутствуют → отказ; не-объекты отклоняются', () => {
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse(copyWithout(VALID_ADD, 'profileId')).success,
    ).toBe(false);
    expect(MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse(copyWithout(VALID_ADD, 'sys')).success).toBe(
      false,
    );
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse(copyWithout(VALID_ADD, 'irregularPulse')).success,
    ).toBe(false);
    expect(
      MEASUREMENT_ADD_REQUEST_SCHEMA.safeParse(copyWithout(VALID_ADD, 'takenAt')).success,
    ).toBe(false);
...
describe('MEASUREMENT_LIST_REQUEST_SCHEMA — query из TASK-021-порта (§5/§11)', () => {
  ...
  it('лимит/смещение — целые неотрицательные; нецелое/отрицательное отклоняется', () => {
    expect(MEASUREMENT_LIST_REQUEST_SCHEMA.safeParse({ profileId: 'p', limit: -1 }).success).toBe(
      false,
    );
    expect(MEASUREMENT_LIST_REQUEST_SCHEMA.safeParse({ profileId: 'p', limit: 10.5 }).success).toBe(
      false,
    );
    expect(MEASUREMENT_LIST_REQUEST_SCHEMA.safeParse({ profileId: 'p', offset: -1 }).success).toBe(
      false,
    );
  });

  it('плохой arm и лишние поля отклоняются; profileId обязателен', () => {
    expect(MEASUREMENT_LIST_REQUEST_SCHEMA.safeParse({ profileId: 'p', arm: 'both' }).success).toBe(
      false,
    );
    expect(
      MEASUREMENT_LIST_REQUEST_SCHEMA.safeParse({ profileId: 'p', order: 'desc' }).success,
    ).toBe(false);
...
describe('MEASUREMENT_UPDATE_REQUEST_SCHEMA — dto+id (§5/§11)', () => {
  ...
  const VALID_UPDATE = copyWithout({ id: VALID_DTO.id, ...VALID_ADD }, 'profileId');
  ...
  it('sys ≤ dia и границы проверяются так же, как в add', () => {
    expect(firstMessage(MEASUREMENT_UPDATE_REQUEST_SCHEMA, { ...VALID_UPDATE, sys: 49 })).toBe(
      'errors.rangeSys',
    );
    expect(
      firstMessage(MEASUREMENT_UPDATE_REQUEST_SCHEMA, { ...VALID_UPDATE, sys: 120, dia: 120 }),
    ).toBe('errors.sysLeDia');
  });

  it('без id → отказ; profileId лишний (strict, §14; перенос между профилями не выражается)', () => {
    expect(
      MEASUREMENT_UPDATE_REQUEST_SCHEMA.safeParse(copyWithout(VALID_UPDATE, 'id')).success,
    ).toBe(false);
    expect(
      MEASUREMENT_UPDATE_REQUEST_SCHEMA.safeParse({ ...VALID_UPDATE, profileId: 'p' }).success,
    ).toBe(false);
  });
});
...
describe('MEASUREMENT_DELETE_REQUEST_SCHEMA — {id} (§5/§11)', () => {
  it('принимает {id}; без id и с лишним полем — отказ', () => {
    ...
    expect(
      MEASUREMENT_DELETE_REQUEST_SCHEMA.safeParse({ id: VALID_DTO.id, profileId: 'p' }).success,
    ).toBe(false);
  });
});
...
describe('MEASUREMENT_DTO_SCHEMA — плоская форма агрегата (§7)', () => {
  ...
  it('pulse/note — опциональны; обязательные поля отсутствуют → отказ', () => {
    const noOptionals = copyWithout(copyWithout(VALID_DTO, 'pulse'), 'note');
    expect(MEASUREMENT_DTO_SCHEMA.safeParse(noOptionals).success).toBe(true);
    expect(MEASUREMENT_DTO_SCHEMA.safeParse(copyWithout(VALID_DTO, 'id')).success).toBe(false);
    expect(MEASUREMENT_DTO_SCHEMA.safeParse(copyWithout(VALID_DTO, 'takenAtUtcMs')).success).toBe(
      false,
    );
  });
...
describe('флаги эвристик в ответе add (§5/§11)', () => {
  it('TypoFlagDto: {field, median, value, deviation}; поле — только sys|dia (TASK-018)', () => {
    ...
    expect(MEASUREMENT_TYPO_FLAG_SCHEMA.safeParse({ field: 'sys', median: 125 }).success).toBe(
      false,
    );
  });
  ...
  it('ответ add: {measurement, flags} — пустые флаги валидны, неизвестные значения отклоняются', () => {
    ...
    expect(
      MEASUREMENT_ADD_RESPONSE_SCHEMA.safeParse({
        ...
        flags: {
          typo: { field: 'dia', median: 80, value: 130, deviation: 50 },
          duplicate: true,
          criticalValue: 'low',
        },
      }).success,
    ...
    expect(
      MEASUREMENT_ADD_RESPONSE_SCHEMA.safeParse({
        measurement: VALID_DTO,
        flags: { typo: undefined, extra: 1 },
      }).success,
    ).toBe(false);
  });

  it('без flags или без measurement → отказ (flags — обязательная часть ответа add)', () => {
    expect(MEASUREMENT_ADD_RESPONSE_SCHEMA.safeParse({ measurement: VALID_DTO }).success).toBe(
      false,
    );
...
describe('ответы list/update/delete (§11)', () => {
  it('list: {items: MeasurementDto[], total} — пустой список и наполненный проходят', () => {
    ...
    expect(
      MEASUREMENT_LIST_RESPONSE_SCHEMA.safeParse({ items: [], total: 0, next: 2 }).success,
    ).toBe(false);
  });

  it('update: {measurement} — только DTO', () => {
    expect(MEASUREMENT_UPDATE_RESPONSE_SCHEMA.safeParse({ measurement: VALID_DTO }).success).toBe(
      true,
    );
    expect(
      MEASUREMENT_UPDATE_RESPONSE_SCHEMA.safeParse({ measurement: VALID_DTO, flags: {} }).success,
    ).toBe(false);
  });
