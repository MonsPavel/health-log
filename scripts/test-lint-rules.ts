...
const EXPECTATIONS: ReadonlyArray<{ file: string; ruleId: string | null }> = [
  {
    file: `${FIXTURE_PREFIX}src-renderer/violation-renderer-node.ts`,
...
function formatMessages(
  messages: ReadonlyArray<{ ruleId: string | null; line?: number; message: string }>,
): string {
  return messages
    .map(
      (m) =>
        `    ${m.line !== undefined ? `:${m.line} ` : ''}[${m.ruleId ?? 'parse'}] ${m.message}`,
    )
    .join('\n');
}
...
async function main(): Promise<number> {
  ...
  try {
    ...
    console.error(
      'test:lint-rules: FAIL: не читается eslint.config.js (RED без конфига ESLint — так и должно быть до GREEN)',
    );
    return 1;
  }
  ...
  const fixtureConfig = toFixtureConfig(raw).filter(
    (block) => !(block.ignores !== undefined && block.files === undefined),
  );
  ...
  for (const expectation of EXPECTATIONS) {
    ...
    if (!hit) {
      failed = true;
      console.error(
        `test:lint-rules: FAIL: ${shortName(expectation.file)} — ожидался error [${expectation.ruleId}]:`,
      );
      console.error(formatMessages(messages));
    } else {
    ...
    if (warnings.length > 0 && !failed) {
      ...
      failed = true;
      console.error(
        `test:lint-rules: FAIL: ${shortName(expectation.file)} — есть warnings (§13: --max-warnings 0):`,
      );
